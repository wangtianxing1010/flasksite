...
def index():
    ...
    # todo ? index() does not take param 'page'
...
def follow(username):
    ...
    if current_user.is_following(user):
        flash(_("You are already following the user."))
        return redirect(url_for('.user', username=username))
    current_user.follow(user)
    db.session.commit()
    flash(_("You are now following the %(username)s.", username=username))
    return redirect(url_for('.user', username=username))
...
def unfollow(username):
    ...
    if user is None:
        ...
        return redirect(url_for('.user', username=current_user.username))
    if current_user == user:
        flash(_("You are not following yourself in the first place."))
        return redirect(url_for('.user', username=username))
...
def search():
    ...
    # todo read pagination again ? search() does not take param q
...
@main_bp.route('/send_messages/<recipient>', methods=["POST", "GET"])
@login_required
def send_messages(recipient):
    ...
    return render_template('send_message.html',
                           title=_("Send Message"), recipient=recipient, form=form)
...
@login_required
def notifications():
    # avoid being notified over and over about the intentially ignored unread message
